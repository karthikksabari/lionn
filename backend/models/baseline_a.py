"""Baseline A: shallow MLP trained on data only (no physics constraint)."""
...
SAVE_PATH = SAVED_DIR / "baseline_a.pt"
...
def train(X_train: np.ndarray, y_train: np.ndarray) -> BaselineA:
    ...
    model = BaselineA(in_features=X.shape[1])
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if epoch % 50 == 0 or epoch == 1:
            print(f"[baseline_a] epoch {epoch:4d}/{EPOCHS}  loss={epoch_loss / len(X):.6f}")

    SAVED_DIR.mkdir(parents=True, exist_ok=True)
    torch.save(model.state_dict(), SAVE_PATH)
    print(f"[baseline_a] saved -> {SAVE_PATH}")
    return model


def load(in_features: int = IN_FEATURES) -> BaselineA:
    model = BaselineA(in_features=in_features)
